...
const forceRefreshChart = () => {
  console.log('强制刷新图表，当前错题数:', wrongQuestions.value.length)
...
// 更新饼图
const updateChart = () => {
  console.log('updateChart 被调用，chartRef.value:', !!chartRef.value, '已有实例:', !!chartInstance)
  ...
  if (chartRef.value.offsetHeight === 0 || chartRef.value.offsetWidth === 0) {
    console.warn('chartRef DOM 尺寸为 0，延迟后重试')
    setTimeout(() => {
      updateChart()
    }, 200)
    return
